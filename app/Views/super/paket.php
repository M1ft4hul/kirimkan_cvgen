...
<body>
    ...
    <nav class="navbar navbar-expand-lg bg-white navbar-light shadow border-top border-5 border-primary sticky-top p-0">
        ...
        <div class="collapse navbar-collapse" id="navbarCollapse">
            <div class="navbar-nav ms-auto p-4 p-lg-0">
                ...
                <a href="<?= base_url('/pakets') ?>" class="nav-item nav-link active">Kirim</a>
    ...
    <div class="container-fluid page-header py-5" style="margin-bottom: 6rem">
        <div class="container py-5">
            <h1 class="display-3 text-white mb-3 animated slideInDown">Kirimkan</h1>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <?php if ($x->status == 0) {
                                            ...
                                            echo '<span class="text-danger">Diterima di Admin Jerman, Paket akan dikirim ke Indonesia setelah pembayaran dinyatakan Valid.</span><br><span class="badge bg-danger">Menunggu pembayaran</span>';
                                        } elseif ($x->status == 2) {
                                            echo '<span class="text-info">Diterima di Admin Indonesia, sedang diproses untuk dikirim ke Alamat tujuan.</span>';
                                        } else {
                                    ...
                                    <td>
                                        Rp. <strong><?= $x->harga ?>,- </strong>
                                    </td>
                                    <td>
                                        <?php if ($x->status == 0) { ?>
                                            <button type="button" class="btn btn-warning btn-sm" onclick="ubah_harga('<?= $x->kode_paket ?>')"><i class="bi bi-pencil-square"></i></button>
                                        <?php } elseif ($x->status == 1) { ?>
                                            <a href="<?= base_url('/bayar/' . $x->kode_paket) ?>" type="button" class="btn btn-info btn-sm">Bayar</a>
    ...
    <div class="modal fade" id="ubah_harga" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form action="<?= base_url() ?>/ubah-paket" method="POST">
                    <div class="modal-body">
                        <p>Masukkan dalam bentuk Angka untuk <strong>Berat dan Biaya Pengiriman.</strong></p>

                        <?= csrf_field() ?>
                        <div class="row g-3">
                            <div class="form-floating">
                                <input type="hidden" name="id_paket" id="id_paket" class="onIDpaket">
                                <input type="number" class="form-control" id="berat" name="berat" placeholder="Berat paket (Kg)" required />
                                <label for="berat">Berat Paket (Kg)</label>
                            </div>
                            <div class="form-floating">
                                <input type="text" class="form-control" id="hargarp" name="hargarp" placeholder="Biaya pengiriman (Rupiah)" required />
                                <label for="harga">Biaya Pengiriman (Rupiah)</label>
                            </div>
                            <div class="form-floating">
                                <input type="text" class="form-control" id="harga" name="harga" placeholder="Biaya pengiriman (Euro)" required />
                                <label for="harga">Biaya Pengiriman (Euro)</label>
                            </div>
                            <div class="form-floating">
                                <input type="datetime-local" class="form-control" id="bayar_sebelum" name="bayar_sebelum" required />
                                <label for="bayar_sebelum">Jatuh Tempo</label>
                            </div>
                        </div>
